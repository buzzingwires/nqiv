                                      ...
                                      void (*print_prefix)(const nqiv_cmd_manager*))
{
	...
	case NQIV_CMD_ARG_PRUNER:
		...
		print_prefix(manager);
		fprintf(stdout, "'loaded_behind' <THRESHOLD> <MAX>\n");
		print_prefix(manager);
		fprintf(stdout, "'bytes_ahead' <THRESHOLD> <MAX>\n");
		print_prefix(manager);
		fprintf(stdout, "'bytes_behind' <THRESHOLD> <MAX>\n");
		print_prefix(manager);
		...
		fprintf(
			stdout,
			"'not_cropped' will check if the currently-selected image is cropped from the full "
		    "image, such as in the case of its dimensions being larger than the texture size.\n");
		manager->print_settings.indent -= 1;
		break;
	case NQIV_CMD_ARG_WORKER_SPEC:
		...
		for(priority = NQIV_EVENT_PRIORITY_FIRST; priority <= NQIV_EVENT_PRIORITY_LAST;
		    ++priority) {
			print_prefix(manager);
			fprintf(stdout, "%s - %s\n", nqiv_event_priority_names[priority],
			        nqiv_event_priority_descriptions[priority]);
		}
		manager->print_settings.indent -= 1;
		break;
	}
